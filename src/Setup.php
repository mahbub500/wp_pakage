...
class Setup extends Base {
    ...
    public function get_logo() {
    	...
    	echo "<img class='wph-wizard-icon' src='{$this->plugin['icon']}' />";
    }
	...
	public function header() {
		...
		<html xmlns="http://www.w3.org/1999/xhtml" <?php language_attributes(); ?>>
			...
			<body class="wph-setup wp-core-ui wph-wizard-body-panel">
				<div class="wph-wizard-wrap">
					<h1 class="wph-wizard-heading <?php echo $hide_title ?>">
					...
					<?php $this->pagination(); ?>
					<div class="wph-wizard-container">
		<?php
	}
	...
	public function pagination() {
		...
		if( count( $this->steps ) > 1 ) {
			...
			echo '<div class="wph-wizard-stepper-wrapper">';
			...
			foreach ( $this->steps as $step => $data ) {
				...
				echo "
				  <div class='wph-wizard-stepper-item wph-step-{$count} {$classes} {$passed}'>
				    <div class='wph-wizard-step-name'><a href='{$url}'>{$data['label']}</a></div>
				    <div class='wph-wizard-step-counter'></div>
				  </div>
				";
		...
		<div class="wph-wizard-content">
			<?php 
			$current_step 	= $this->current_step();
			$action 		= add_query_arg( 'saved', 1, $this->get_step_url( $current_step ) );
			echo "<form id='wph-{$current_step}-form' method='POST' action='{$action}'>";
			?>
		<div class="wph-wizard-page">
		<?php
	}
	...
	public function footer() {
		...
		?>
									<div class="wph-wizard-btns">
										...
										$prev_text 		= isset( $config['prev_text'] ) ? $config['prev_text'] : __( 'Previous', 'wph-plugin' );
										$next_text		= $current_step == $next_step ? __( 'Finish', 'wph-plugin' ) : ( isset( $config['next_text'] ) ? $config['next_text'] : __( 'Next', 'wph-plugin' ) );

										echo "<a class='wph-wizard-btn btn-hero prev {$disabled}' href='{$prev_step_url}'>{$prev_text}</a>";

										if( ! isset( $config['next_url'] ) ) {
											echo "<button id='{$current_step}-btn' class='wph-wizard-btn btn-hero btn-primary next'>{$next_text}</button>";
										}
										else {
											echo "<a id='{$current_step}-btn' class='wph-wizard-btn btn-hero btn-primary next' href='{$config['next_url']}'>{$next_text}</a>";
										}
										?>
									</div>
								</div>
